...
class ReportRowRepository(AbstactBaseRepository):
    ...
    async def get_unique_row_count_by_report_id(self, report_id: int) -> int:
        statement = await self.session.execute(
            select(func.count(self.model)).where(self.model.value.isnot(None), self.model.report_id == report_id)
        )
        return statement.scalar()

    async def get_nullable_row_count_by_report_id(self, report_id: int) -> int:
        statement = await self.session.execute(
            select(func.count(self.model).where(self.model.value.is_(None), self.model.report_id == report_id))
        )
        return statement.scalar()

    async def get_updated_row_count_by_report_id(self, report_id: int) -> int:
        statement = await self.session.execute(
            select(func.count(self.model).where(self.model.updated_at.isnot(None), self.model.report_id == report_id))
        )
        return statement.scalar()

    async def get_deleted_row_count_by_report_id(self, report_id: int) -> int:
        statement = await self.session.execute(
            select(func.count(self.model).where(self.model.is_deleted.is_(True), self.model.report_id == report_id))
        )
        return statement.scalar()
